(function(){
  const STORAGE_KEY = 'evercrestOwnerThemeProV1';
  const defaults = {
    preset: 'forest-studio',
    mode: 'dark',
    accent: '#c7b26a',
    accentAlt: '#7dd3a0',
    radius: 22,
  ...
  const presets = {
    'nova-original': { label:'Evercrest Noir', values:{'--bg-main':'#0b1120','--bg-panel':'#111827','--bg-soft':'#1e2d45','--text-main':'#ffffff','--text-soft':'rgba(255,255,255,0.78)','--text-muted':'rgba(255,255,255,0.62)','--text-faint':'rgba(255,255,255,0.48)','--accent':'#f59e0b','--accent-hover':'#fb923c','--accent-alt':'#38bdf8','--border':'rgba(255,255,255,0.1)','--border-light':'rgba(255,255,255,0.06)','--bg-gradient-start':'#435065','--bg-gradient-end':'#1a2c44','--announcement-start':'#f59e0b','--announcement-end':'#fb923c'}},
  ...
  function ensureHeaderSettingsLink(){
    ...
    const label = window.NovaBrandSettings?.read?.().copyFooterOwnerButton || 'Store Settings';
