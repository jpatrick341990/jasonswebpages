document.addEventListener("DOMContentLoaded", () => {
  ...
  const PROFILE_KEY = "novaMarketDemoProfile";
  const ADDRESS_KEY = "novaMarketDemoAddresses";
  const CONTACT_KEY = "novaMarketDemoMessages";
  const NEWSLETTER_KEY = "novaMarketDemoNewsletter";
  const SEARCH_RECENTS_KEY = "novaMarketDemoSearchRecents";
  const SELLER_PROFILE_KEY = "novaMarketDemoSellerProfileV1";
  const BLOG_POSTS = {
    ...
    "A Simple Content Plan for Small Online Shops": {
      ...
      body: "An easy rhythm is: one product spotlight, one how-to, one customer question answer, and one seasonal feature each month. That gives the blog page real purpose instead of looking like filler. This starter article is intentionally written in a simple format so the template buyer can replace it in minutes.",
  ...
  function sellerProfileDefaults() {
    return {
      storeName: 'Nova Select Studio',
      supportEmail: 'seller@novamarket.example',
  ...
  function productCard(product, compact = false) {
    const safeProduct = {
      ...
      badge: product?.badge || "",
      ownerName: product?.ownerName || "Nova Select"
    };
  ...
  function renderProductPage() {
    ...
    document.title = `${product.title} | Nova Market`;
  ...
  function renderSellerDashboard() {
    ...
    const demoOrdersFallback = [
      { id:'NM-482193', createdAt:new Date(Date.now()-86400000*2).toISOString(), status:'Packed', customer:{name:'Maya Carter', email:'maya@shoppermail.example'}, total:186.00 },
      { id:'NM-482118', createdAt:new Date(Date.now()-86400000*3).toISOString(), status:'Shipped', customer:{name:'Ethan Webb', email:'ethan@shoppermail.example'}, total:74.00 },
      { id:'NM-481990', createdAt:new Date(Date.now()-86400000*5).toISOString(), status:'Confirmed', customer:{name:'Lena Scott', email:'lena@shoppermail.example'}, total:242.00 }
    ];
  ...
  document.addEventListener('nova-brand-updated', updateHeaderCounts);
    ...
    resetCatalogButton?.addEventListener('click', () => {
      const okay = window.confirm('Reset the catalog back to the original 16 starter products? This removes any custom products saved in this browser.');
      if (!okay) return;
  ...
  document.addEventListener('nova-brand-updated', updateHeaderCounts);
      ...
      renderCollections();
      renderShop();
      showToast('Catalog reset to the original 16 starter products.');
    });
  ...
  function renderCheckoutPage() {
    ...
    if (summaryCard) {
      summaryCard.innerHTML = `<p class="section-kicker">Order Summary</p><h3>Review Before Purchase</h3><p>Review your order before purchase, then place the order to save it in the account area and tracking page.</p><div class="checkout-summary-list">${metrics.lines.length ? metrics.lines.map((line) => `<div class="cart-item"><span>${escapeHtml(line.product.title)} × ${line.quantity}</span><span>${formatPrice(line.product.price * line.quantity)}</span></div>`).join('') : '<div class="cart-item"><span>No items in cart</span><span>$0.00</span></div>'}<div class="cart-item"><span>Subtotal</span><span>${formatPrice(metrics.subtotal)}</span></div><div class="cart-item"><span>Shipping</span><span>${metrics.shipping ? formatPrice(metrics.shipping) : 'Free'}</span></div><div class="cart-item"><span>Tax</span><span>${formatPrice(metrics.tax)}</span></div><div class="cart-item total-row"><span>Total</span><span>${formatPrice(metrics.total)}</span></div></div><div class="notice-card mt-24"><p><strong>Security:</strong> This is still a front-end checkout preview, but it saves billing details and completed saved orders locally.</p></div><button ${metrics.lines.length ? '' : 'disabled'} class="btn btn-primary full-btn" id="placeOrderButton" type="button">${metrics.lines.length ? 'Place Order' : 'Cart Empty'}</button><p class="seller-status" id="checkoutStatus"></p>`;
    }
  ...
  function renderContactPage() {
    ...
    form.addEventListener('submit', (e) => {
      ...
      writeJson(CONTACT_KEY, messages); form.reset(); document.getElementById('contactStatus').textContent = 'Message sent. Your request has been saved in this storefront preview.'; showToast('Message sent.');
    });
  }
